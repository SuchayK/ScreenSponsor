...
import { qualityGate } from "./job-logic";
...
export function createJob(uploadId?: string) {
  const id = randomUUID();
...
async function render(job: JobView) {
  ...
  const source = job.artifacts.original?.startsWith("/api/artifacts/source/")
    ? state.uploads.get(job.artifacts.original.split("/").pop()!)!
    : path.join(process.cwd(), "public", "demo", "original.mp4");
...
async function processJob(id: string) {
  ...
  try {
    await wait(450); advance(job,"analyzing",16,"Reading the scene","Video model inspected composition, people, text, and available surfaces.","Fireworks");
    await wait(650); job.candidates = [candidate(), {...candidate(), id:randomUUID(), mode:"counter", confidence:.73, occlusionRisk:"medium", rationale:"Counter edge is usable but partially leaves frame."}]; advance(job,"proposing",28,"Two surfaces proposed","Wall plane ranked above counter edge for stability and visual fit.","Fireworks");
