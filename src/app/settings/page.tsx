...
export default function SettingsPage(){const [fireworksEnabled,setFireworksEnabled]=useState(false);useEffect(()=>setFireworksEnabled(localStorage.getItem(FIREWORKS_SETTING)==="true"),[]);const setFireworks=(enabled:boolean)=>{localStorage.setItem(FIREWORKS_SETTING,String(enabled));setFireworksEnabled(enabled)};return <SectionShell code="ST/CO" eyebrow="CREATOR CONTROLS" title="Settings" description="Set the boundaries the agent must respect before it analyzes or modifies a clip." asideTitle="Creator rules lead" asideCopy="These controls are a frontend preview. They show how creator preferences will govern matching and export.">
